...
// ── Memory leak completion helper ──
function completeMemoryLeak(ctx, bugId, bug) {
  const { state: st } = ctx;
  ...
  const allHolders = bug.holders ? Array.from(bug.holders.keys()) : [];
    ...
    switch (msg.type) {
      ...
      case 'click-memory-leak-start': {
        ...
        if (!bug.holders.has(pid)) {
          ...
          const remaining = Math.max(0, effectiveRequiredTime - elapsedSinceFirst);

          // Set/reset auto-complete timer
          if (bug.autoCompleteTimer) clearTimeout(bug.autoCompleteTimer);
          bug.autoCompleteTimer = setTimeout(() => {
            if (!st.bugs[bugId] || !bug.holders) return;
            completeMemoryLeak(ctx, bugId, bug);
          }, remaining);
      ...
      case 'click-memory-leak-complete': {
        ...
        const requiredTime = MEMORY_LEAK_CONFIG.holdTimeByStage[bug.holdStartStage];
        const holderCount = bug.holders.size;
        
        // With multiple holders, effective required time for collective completion
        // Each holder still needs to hold their full time, but collectively they clear it faster
        const elapsedSinceFirst = Date.now() - bug.firstHolderStartTime;
        const effectiveRequiredTime = requiredTime / holderCount;
        
        // Check if this player held long enough personally
        if (playerHoldDuration < effectiveRequiredTime) {
          // Released too early - remove this holder
          ...
          if (bug.holders.size === 0) {
            if (bug.autoCompleteTimer) { clearTimeout(bug.autoCompleteTimer); bug.autoCompleteTimer = null; }
            delete bug.holders;
            delete bug.holdStartStage;
            delete bug.firstHolderStartTime;
            // Resume growth
            if (!bug.growthInterval && bug.growthStage < MEMORY_LEAK_CONFIG.maxGrowthStage) {
              const phaseCheck = st.phase;
              bug.growthInterval = setInterval(() => {
                if (st.phase !== phaseCheck || !st.bugs[bugId]) return;
                if (bug.growthStage < MEMORY_LEAK_CONFIG.maxGrowthStage) {
                  bug.growthStage++;
                  network.broadcastToLobby(ctx.lobbyId, { type: 'memory-leak-grow', bugId, growthStage: bug.growthStage });
                }
              }, MEMORY_LEAK_CONFIG.growthInterval);
            }
          } else {
            // Recalculate auto-complete timer with fewer holders
            const newEffective = requiredTime / bug.holders.size;
            const newRemaining = Math.max(0, newEffective - elapsedSinceFirst);
            if (bug.autoCompleteTimer) clearTimeout(bug.autoCompleteTimer);
            bug.autoCompleteTimer = setTimeout(() => {
              if (!st.bugs[bugId] || !bug.holders) return;
              completeMemoryLeak(ctx, bugId, bug);
        ...
        // Check if collective progress is complete
        if (elapsedSinceFirst < effectiveRequiredTime) {
          // Not done yet, just ignore
          break;
        }

        // Success! Clear the leak
        completeMemoryLeak(ctx, bugId, bug);
        break;
      }
...
async function handleLeaveLobby(ws, pid, lobbyId) {
  ...
  if (mem) {
    ...
    // Clean up memory leak holders for the departing player
    for (const bugId of Object.keys(st.bugs)) {
      const bug = st.bugs[bugId];
      if (!bug.isMemoryLeak || !bug.holders || !bug.holders.has(pid)) continue;

      bug.holders.delete(pid);
      const ctx = { lobbyId, state: st, counters: mem.counters, timers: mem.timers, playerInfo };
      Object.defineProperty(ctx, 'matchLog', {
        get() { return mem.timers._matchLog || null; },
        set(v) { mem.timers._matchLog = v; },
        enumerable: true,
      ...
      if (bug.holders.size === 0) {
        // No holders left — clear auto-complete and resume growth
        if (bug.autoCompleteTimer) { clearTimeout(bug.autoCompleteTimer); bug.autoCompleteTimer = null; }
        delete bug.holders;
        delete bug.holdStartStage;
        delete bug.firstHolderStartTime;
        ...
        const requiredTime = MEMORY_LEAK_CONFIG.holdTimeByStage[bug.growthStage];
        network.broadcastToLobby(lobbyId, {
          type: 'memory-leak-hold-update',
          bugId,
          playerId: pid,
    ...
    const remaining = Object.keys(st.players).length;
    ...
    if (remaining === 0) {
      game.resetToLobby({ lobbyId, state: st, counters: mem.counters, timers: mem.timers });
    }
  }
}
